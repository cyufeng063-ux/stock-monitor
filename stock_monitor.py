...
def _fetch_wencai_review(codes: list[str]) -> dict[str, dict]:
    """从同花顺问财尝试抓取每只股票的当日复盘指标：
    散户指数、筹码集中度、主力控盘力度、平均成本
...
def build_html(quotes: list[dict], announcements: list[dict],
               ...
               kline: dict[str, dict] = None,
               reviews: dict[str, dict] = None) -> str:
    """拼装 HTML 邮件正文。"""
...
def run_collect(config: dict) -> str:
    ...
    # 当日复盘 — 同花顺问财（每日18:00更新）
    try:
        print("[.] 拉取同花顺问财当日复盘指标...")
        reviews = _fetch_wencai_review(codes)
    except Exception as e:
        print(f"  问财复盘抓取失败: {e}")
        reviews = {}
    ...
    return build_html(quotes, announcements, interpretations, exp_dates, kline, reviews)
